...
tofucode supports a bring-your-own-bot Slack integration, letting you interact with Claude Code directly from Slack channels and DMs. The bot classifies incoming messages and responds or starts a work session — all without leaving Slack.

### How It Works

- **Channel messages** — the bot reads messages in configured channels and classifies them: ignore, acknowledge, answer, create a ticket, or start a code session
- **DMs** — direct messages to the bot start a persistent Claude Code session; each conversation thread continues in context
- **Two-tier classification** — Haiku runs first (fast, cheap); uncertain messages escalate to Sonnet with Grep/Glob/Read tools to investigate the codebase before responding
- **Sessions shared with Web UI** — work sessions started from Slack are visible and continuable in the tofucode Web UI

### Setup

1. Create a Slack app at [api.slack.com/apps](https://api.slack.com/apps) with:
   - **Bot Token Scopes:** `channels:history`, `chat:write`, `reactions:write`, `users:read`, `groups:history`, `im:history`, `mpim:history`
...
2. Add your credentials to `.env`:
   ...
   ```
3. Configure the bot in **Settings → Slack** in the tofucode Web UI

### Configuration (Settings → Slack)

| Setting | Description |
|---------|-------------|
| **Bot User ID** | Your bot's Slack user ID (e.g. `U0XXXXXXXXX`) — used to detect mentions and avoid self-responses |
| **Project Root Path** | Absolute path to your projects folder — the bot maps work requests to subfolders here |
| **Session Log Path** | Absolute path where classifier sessions are persisted — enables session continuity and CLAUDE.md context loading |
| **Respond to DMs** | Whether to respond to direct messages to the bot |
| **Debounce Window** | Milliseconds to wait before processing a burst of messages (default: 10000ms) — groups rapid messages into one |
| **Max Triage Turns** | Maximum tool-call turns for the agentic Sonnet triage pass (default: 5) |
| **Classifier System Prompt** | Optional full override of the classifier's system prompt — use to inject your identity, role, tone, and domain context |

### Classifier Context via CLAUDE.md

The bot's classifier supports an optional `CLAUDE.md` file for injecting project-specific context without modifying the system prompt — useful for service descriptions, routing conventions, team norms, etc.

**Requirements:**
- **Session Log Path** must be set in Settings
- A `.claude/` directory must exist in that path (can be empty)
- A `CLAUDE.md` file in the session log path (either `<path>/CLAUDE.md` or `<path>/.claude/CLAUDE.md`)

**Example:**
```
~/slack-sessions/
├── .claude/          ← required (can be empty)
└── CLAUDE.md         ← your classifier context
```

**What to put in it:** service descriptions, API path conventions, which projects own which domains, team member roles, known patterns. Keep it focused — this file is loaded for every message classification, so keep it concise and classifier-relevant.

> **Note:** The global `~/.claude/CLAUDE.md` and the tofucode project's own `CLAUDE.md` are intentionally excluded from the classifier to keep it lean and focused.

### Key Files

| File | Purpose |
|------|---------|
| `server/slack/lib/classifier.js` | Two-tier classification engine (Haiku → Sonnet) |
| `server/slack/lib/debounce.js` | Message debouncing — groups rapid messages per user per thread |
| `server/slack/events/message.js` | Channel message handler |
| `server/slack/events/dm.js` | DM handler — persistent per-user sessions |
| `server/slack/events/shared.js` | Shared dispatch logic (work/ticket/answer actions) |
| `server/slack/lib/formatter.js` | Thread context formatting for classifier prompts |
| `server/slack/config.js` | Default config schema |
| `server/events/slack.js` | WebSocket event handler for Slack config save/load |
